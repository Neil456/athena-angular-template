/**
 * Angular Webpack Component Tagger for Athena.sh
 * Adds data-athena-id and data-athena-name attributes to Angular components
            ...
            if (pathname.endsWith('.js')) {
              ...
              componentsFound.forEach((componentName, selector) => {
                ...
                content = content.replace(templatePattern, (match, elementStart) => {
                  ...
                  const dataAttributes = `
                    ɵɵattribute("data-athena-id", "${filePath}:0:0");
                    ɵɵattribute("data-athena-name", "${componentName}");
                  `;
              ...
              content = content.replace(elementPattern, (match, tagName, attributes) => {
                if (componentsFound.has(tagName)) {
                  ...
                  if (!attributes.includes('data-athena-id')) {
                    return `<${tagName} data-athena-id="${filePath}:0:0" data-athena-name="${componentName}"${attributes}>`;
                  }
                }
                return match;
              });
